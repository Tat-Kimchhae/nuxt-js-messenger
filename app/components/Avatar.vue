...
<template>
  <div class="relative shrink-0">
    <div
      class="grid place-items-center rounded-full font-mono font-medium tracking-[-.08em] text-void"
    ...
    >{{ person.initials }}</div>
    <span v-if="showPresence && person.online" class="presence-pulse absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-panel bg-mint" />
  </div>
</template>